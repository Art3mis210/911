...
public class Enemy_Coop_LineOfSight : MonoBehaviour
{
    public GameObject Enemy;
    public AudioClip Alert;
    private void Update()
    {
        if (Enemy.GetComponent<Enemy_Coop>().FoundPlayer == true)
            Destroy(gameObject);
        if (Enemy.GetComponent<Enemy_Coop>().Dead == true)
            Destroy(gameObject);
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "Player")
        {
            ...
            Enemy.GetComponent<Enemy_Coop>().FoundPlayer = true;
        }
    }
}
