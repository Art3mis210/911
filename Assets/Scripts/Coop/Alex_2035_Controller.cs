...
public class Alex_2035_Controller : MonoBehaviour
{
    ...
    private void Movement()
    {
        ...
        if (((Input.GetKeyDown("joystick button 1") && animator.GetBool("Shoot") == false)))
        {
            animator.SetBool("Stand", !animator.GetBool("Stand"));

        }
        if ((Input.GetKey("joystick button 8")) && animator.GetBool("Move") == true)
        {
        ...
        if (animator.GetBool("Move") == false && Input.GetAxis("SHOOT") > 0)
        {
            animator.SetBool("Shoot", true);
        }
        else
            animator.SetBool("Shoot", false);
        if (animator.GetBool("Move") == false && Input.GetAxis("MELEE") > 0)
        {
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            Physics2D.IgnoreCollision(collision.gameObject.GetComponent<BoxCollider2D>(), boxC);
        }
    }
